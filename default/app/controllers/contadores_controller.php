...
class ContadoresController extends AppController
{
    ...
    {
        View::template(null);
         $this->listRegistros = (new Contadores())->getRegistros($page);
    }
    ...
    public function crear()
    {
        ...
        if (Input::hasPost('registros')) {
            ...
            $registro = new Contadores(Input::post('registros'));
    ...
    public function editar($id){
        ...
        $buscado = (new Contadores())->find_by_id((int)$id);

        if($buscado != NULL){
            $registro = new Contadores();
            //verificar si se ha enviado el formulario
            if(Input::hasPost('registros')){
                if($registro->update(Input::post('registros'))){
                    Flash::valid('Operacion exitosa');
                    return Redirect::to('contadores/listar');
                }
    ...
    public function ver($id){
        View::template(null);
        $this->buscado = (new Contadores())->find_by_id((int)$id);
    }
}
