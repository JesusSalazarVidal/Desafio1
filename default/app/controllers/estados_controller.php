...
class EstadosController extends AppController
{
    ...
    {
        View::template(null);
         $this->listRegistros = (new Estados())->getRegistros($page);
    }
    ...
    public function crear()
    {
        ...
        if (Input::hasPost('registros')) {
            ...
            $registro = new Estados(Input::post('registros'));
    ...
    public function editar($id){
        ...
        $buscado = (new Estados())->find_by_id((int)$id);

        if($buscado != NULL){
            $registro = new Estados();
            //verificar si se ha enviado el formulario
            if(Input::hasPost('registros')){
                if($registro->update(Input::post('registros'))){
                    Flash::valid('Operacion exitosa');
                    return Redirect::to('Estados/listar');
                }
    ...
    public function ver($id){
        View::template(null);
        $this->buscado = (new Estados())->find_by_id((int)$id);
    }
}
