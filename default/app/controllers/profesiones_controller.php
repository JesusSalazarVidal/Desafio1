...
class ProfesionesController extends AppController
{
    ...
    {
        View::template(null);
         $this->listRegistros = (new Profesiones())->getRegistros($page);
    }
    ...
    public function crear()
    {
        ...
        if (Input::hasPost('registros')) {
            ...
            $registro = new Profesiones(Input::post('registros'));
    ...
    public function editar($id){
        ...
        $buscado = (new Profesiones())->find_by_id((int)$id);

        if($buscado != NULL){
            $registro = new Profesiones();
            //verificar si se ha enviado el formulario
            if(Input::hasPost('registros')){
                if($registro->update(Input::post('registros'))){
                    Flash::valid('Operacion exitosa');
                    return Redirect::to('profesiones/listar');
                }
    ...
    public function ver($id){
        View::template(null);
        $this->buscado = (new Profesiones())->find_by_id((int)$id);
    }
}
