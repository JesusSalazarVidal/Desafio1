...
class AccesosController extends AppController
{
    ...
    {
        View::template(null);
         $this->listRegistros = (new Accesos())->getRegistros($page);
    }
    ...
    public function crear()
    {
        ...
        if (Input::hasPost('registros')) {
            ...
            $registro = new Accesos(Input::post('registros'));
    ...
    public function editar($id){
        ...
        $buscado = (new Accesos())->find_by_id((int)$id);

        if($buscado != NULL){
            $registro = new Accesos();
            //verificar si se ha enviado el formulario
            if(Input::hasPost('registros')){
                if($registro->update(Input::post('registros'))){
                    Flash::valid('Operacion exitosa');
                    return Redirect::to('accesos/listar');
                }
    ...
    public function ver($id){
        View::template(null);
        $this->buscado = (new Accesos())->find_by_id((int)$id);
    }
}
